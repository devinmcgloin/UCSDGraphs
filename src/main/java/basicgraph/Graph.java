...
/**
 * An abstract class that implements a directed graph. The graph may have self-loops, parallel edges. Vertices are
 * labeled by integers 0 .. n-1 and may also have String labels. The edges of the graph are not labeled. Representation
 * of edges is left abstract.
 *
...
public abstract class Graph {
    ...
    public static void main(String[] args) {
        GraphLoader.createIntersectionsFile("/Users/devinmcgloin/projects/UCSDGraphs/data/maps/ucsd.map", "/Users/devinmcgloin/projects/UCSDGraphs/data/intersections/myucsd.intersections");
        ...
        GraphLoader.loadRoadMap("/Users/devinmcgloin/projects/UCSDGraphs/data/testdata/simpletest.map", graphFromFile);
        ...
        GraphLoader.loadRoutes("/Users/devinmcgloin/projects/UCSDGraphs/data/airports/routesUA.dat", airportGraph);
    ...
    /**
     * Add new vertex to the graph.  This vertex will have as its index the next available integer. Precondition:
     * contiguous integers are used to index vertices.
     *
    ...
    /**
     * The degree sequence of a graph is a sorted (organized in numerical order from largest to smallest, possibly with
     * repetitions) list of the degrees of the vertices in the graph.
     *
     * @return The degree sequence of this graph.
     */
    public List<Integer> degreeSequence() {
        List<Integer> degreeSeq = new ArrayList<>();
        for ( Integer v = 0; v <= getNumVertices() - 1; v++){
            List<Integer> neigh = getInNeighbors(v);
            int degree = 0;
            if(neigh.contains(v))
                degree += 1;
            degree += neigh.size() + getNeighbors(v).size();
            degreeSeq.add(degree);
        }
        Collections.sort(degreeSeq, (o1, o2) -> o2 - o1);
        return degreeSeq;
    }
    ...
    /**
     * Create a new map of vertex indices to string labels (Optional: only if using labeled vertices.)
     */
