...
public class GoogleMap extends JavascriptObject {
    ...
    /**
     * Returns the LatLongBounds of the visual area. Note: on zoom changes the bounds are reset after the zoom event is
     * fired, which can cause unexpected results.
     *
     * @return
     */
    ...
    /**
     * Returns the screen point for the provided LatLong. Note: Unexpected results can be obtained if this method is
     * called as a result of a zoom change, as the zoom event is fired before the bounds are updated, and bounds need to
     * be used to obtain the answer!
     * <p/>
...
//        System.out.println("panBy x: " + x + ", y: " + y);
     ...
     * <p/>
     * We could allow this to handle any state event by adding a parameter JavascriptObject obj, but we would then need
     * to loosen up the event type and either accept a String value, or fill an enum with all potential state events.
     *
     * @param type Type of the event to register against.
     * @param h    Handler that will be called when the event occurs.
     */
