 ...
 * <p/>
 * An instance of this class is assigned to the document model so that Javascript can access it to make callbacks when
 * events occur.
 * <p/>
 * An application calls on one of the two addXXEventHandler methods in the {@link gmapsfx.javascript.object.GoogleMap}
 * class to register an instance of the {@link GFXEventHandler} interface against a certain event type in the Google
 * Maps event model. See addUIEventHandler and addStateEventHandler.
 * <p/>
 * This class uses a key based on a UUID to map the Javascript event handlers registered against the Google Maps model
 * back to our Java event handlers stored here. The addXXEventHandler methods in GoogleMap create functions that call
 * back into this class using the supplied key.
 * <p/>
 * Currently an instance of this class is registered as: <code>document.jsHandlers</code>
 * <p/>
...
public class EventHandlers {
    ...
    /**
     * This method is called from Javascript, passing in the previously created callback key, and the event object. The
     * current implementation is receiving the LatLng object that the Javascript MouseEvent contains.
     * <p/>
     * It may be more useful to return the MouseEvent and let clients go from there, but there is only the stop() method
     * on the MouseEvent?
     *
    ...
    /**
     * This method is called from Javascript, passing in the previously created callback key. It uses that to find the
     * correct handler and then passes on the call. State events in the Google Maps API don't pass any parameters.
     *
     * @param callbackKey Key generated by the call to registerHandler.
     */
